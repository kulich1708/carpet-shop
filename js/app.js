 ...
 */
 ...
 // Fix #435
                    ...
                    // Don't move the document when pressing arrow keys on focused handles
                        ...
                        horizontalKeys.reverse(); else if (options.ort && !options.dir) {
                        ...
                        // End key
                        to = options.spectrum.xVal[options.spectrum.xVal.length - 1]; else 
                        // Home key
 ...
 // CONCATENATED MODULE: ./src/js/files/functions.js
        ...
        /* Проверка поддержки webp, добавление класса webp или no-webp для HTML */        function isWebp() {
        ...
        /* Проверка мобильного браузера */        let isMobile = {
            Android: function() {
        ...
        /* Добавление класса touch для HTML если браузер мобильный */        function addTouchClass() {
                    ...
                    if (spollerTitles.length) {
                        ...
                        spollerTitles.forEach((spollerTitle => {
                            if (hideSpollerBody) {
                                ...
                                if (!spollerTitle.hasAttribute("data-tabs-title")) spollerTitle.nextElementSibling.hidden = false; else {
                                    let spollerTitleParent = spollerTitle.closest("nav");
                                    let spollerBody = spollerTitleParent.nextElementSibling.children[[ ...spollerTitleParent.children ].indexOf(spollerTitle)].children[0];
...
*/        function tabs() {
                ...
                tabsMediaArray.forEach((tabsMediaItem => {
                    ...
                    tabsContentItems.forEach(((tabsContentItem, index) => {
                        if (matchMedia.matches) {
                            if (tabsContentItem.closest("[data-tabs]") && tabsContentItem.closest("[data-tabs]").hasAttribute("data-spollers")) {
                                tabsContent.append(tabsContentItem);
                                tabsContentItem.prepend(tabsTitleItems[index]);
                            }
                            tabsContentItem.hidden = false;
                            tabsMediaItem.classList.add("_tab-spoller");
                        } else {
                            if (tabsContentItem.closest("[data-tabs]") && tabsContentItem.closest("[data-tabs]").hasAttribute("data-spollers")) tabsTitles.append(tabsTitleItems[index]);
                            tabsContentItem.hidden = true;
                            tabsMediaItem.classList.remove("_tab-spoller");
                            tabsMediaItem.querySelector("._tab-active").click();
                        }
                    }));
                }));
            }
            // Работа с контентом
                ...
                if (tabsContent.length) {
                    ...
                    tabsTitles = Array.from(tabsTitles).filter((item => item.closest("[data-tabs]") === tabsBlock && !item.hasAttribute("data-no-tabs-title")));
            ...
            function setTabsAction(e) {
                ...
                if (el.closest("[data-tabs-title]")) {
                    ...
                    if (!el.closest("._tab-spoller")) {
                        if (!tabsBlock.querySelector("._slide") || !tabsBlock.hasAttribute("data-tabs-animate")) setTabsStatus(tabsBlock);
                        e.preventDefault();
                    } else if (!el.closest("._tab-spoller").hasAttribute("data-spollers")) {
                        const tabsItems = tabsBlock.querySelectorAll(".tabs-account__item");
                        const titleItems = tabsBlock.querySelectorAll(".tabs-account__title[data-tabs-title]");
                        tabsItems.forEach(((tabsItem, index) => {
                            if (titleItems[index].classList.contains("_tab-active")) {
                                tabsItem.classList.add("_active");
                                tabsBlock.style.height = tabsItem.scrollHeight + "px";
                                tabTitle.closest("[data-tabs-titles]").classList.add("_opacity");
                            }
                        }));
                    }
                }
...
*/        function menuInit() {
            if (document.querySelector(".icon-menu")) document.addEventListener("click", (function(e) {
                if (bodyLockStatus && (e.target.closest(".icon-menu") || e.target.closest(".actions__column_menu"))) {
                    bodyLockToggle();
...
*/        function showMore() {
                ...
                function getHeight(showMoreBlock, showMoreContent) {
                    ...
                    if (showMoreType === "items") {
                        ...
                        for (let index = 1; index < showMoreItems.length; index++) {
                            ...
                            showMoreItem.classList.remove("last-spoller-view");
                            if (index == showMoreTypeValue) showMoreItem.classList.add("last-spoller-view");
        ...
        class Popup {
            ...
            eventsPopup() {
                ...
                document.addEventListener("click", function(e) {
                    ...
                    if ((this.targetOpen.element && !this.targetOpen.element.hasAttribute("data-not-close") || !this.targetOpen.element) && (buttonClose || !e.target.closest(`.${this.options.classes.popupContent}`) && this.isOpen)) {
                        e.preventDefault();
                        this.close();
                        return;
                    }
                ...
                // Закрытие по ESC
                    ...
                    if ((this.targetOpen.element && !this.targetOpen.element.hasAttribute("data-not-close") || !this.targetOpen.element) && this.options.closeEsc && e.which == 27 && e.code === "Escape" && this.isOpen) {
                        e.preventDefault();
                        this.close();
                        return;
                    }
        ...
        // CONCATENATED MODULE: ./src/js/files/user-info.js
        if (!localStorage.getItem("carpet-shop")) localStorage.setItem("carpet-shop", JSON.stringify({
            user: {
                address: {},
                img: "img/common/not-photo.png",
                payment: {}
            }
        }));
        ...
        if (citySearch) citySearch.addEventListener("keyup", (e => {
        ...
        window.updateAccountInfo = changedData => {
            ...
            if (!changedData) accountInfoBlocks = document.querySelectorAll("[data-get-user-info]"); else changedData.forEach((element => {
            ...
            accountInfoBlocks.forEach((accountInfoBlock => {
                ...
                for (const type of accountInfo) if (userInfo) userInfo = userInfo[type];
                if (userInfo) {
                    if (accountInfoLenght) {
                        accountInfoLenght = accountInfoLenght.split(",").map((e => e.trim()));
                        if (accountInfoLenght[1] == "last") userInfo = userInfo.slice(-accountInfoLenght[0]); else userInfo = userInfo.slice(0, +accountInfoLenght[0]);
                    }
                    if (accountInfoBlock.tagName == "INPUT") accountInfoBlock.value = userInfo; else if (accountInfoBlock.tagName == "IMG") {
                        accountInfoBlock.src = userInfo;
                        if (accountInfoBlock.closest("picture")) accountInfoBlock.closest("picture").querySelector("source").setAttribute("srcset", userInfo);
                    } else if (accountInfoBlock.tagName == "SELECT") {
                        const options = accountInfoBlock.querySelectorAll(`option`);
                        options.forEach((option => {
                            option.removeAttribute("selected");
                            if (option.value == userInfo) option.setAttribute("selected", "");
                        }));
                    } else accountInfoBlock.innerHTML = userInfo;
                } else if (accountInfoBlock.tagName == "INPUT") accountInfoBlock.value = ""; else if (accountInfoBlock.tagName == "SELECT") {
                    const options = accountInfoBlock.querySelectorAll(`option`);
                    options.forEach((option => {
                        option.removeAttribute("selected");
                        option.value == "" ? option.setAttribute("selected", "") : null;
                    }));
                } else accountInfoBlock.innerHTML = "";
            }));
            userName();
        };
        function addAccountLineData(block, data, type) {
            ...
            if (data && item) for (let index = 0; index < data; index++) {
        ...
        function showAccountItem() {
            ...
            itemBlocks.forEach((itemBlock => {
                ...
                if (itemType) {
                    ...
                    } else itemBlock.classList.add("_empty");
                    const tab = itemBlock.closest(".tabs-account");
                    const tabItemAcitve = itemBlock.closest(".tabs-account__item._active");
                    if (tab && tabItemAcitve && document.documentElement.offsetWidth < 767.98) tab.style.height = tabItemAcitve.offsetHeight + "px";
                }
            }));
        }
        showAccountItem();
        ...
        window.openChangeForm = (form, num) => {
            ...
            if (tab && tabItem && document.documentElement.offsetWidth < 767.98) tab.style.height = tabItem.offsetHeight + "px";
        };
        window.closeChangeForm = form => {
            ...
            if (formTitle) formTitle.classList.add("_disabled");
            const changedData = [];
        ...
        function userName() {
            ...
            if (userNameBLock) if (!userData["first_name"] && !userData["last_name"]) userNameBLock.classList.add("_empty"); else userNameBLock.classList.remove("_empty");
        }
        userName();
 ...
 // CONCATENATED MODULE: ./src/js/files/scroll/gotoblock.js
        ...
        // Модуль плавной проктутки к блоку
            ...
            if (targetBlockElement) {
                ...
                (new SmoothScroll).animateScroll(targetBlockElement, "", options); else {
...
*/
        ...
        function formFieldsInit(options = {
            ...
            if (formFields.length) formFields.forEach((formField => {
                if (!formField.hasAttribute("data-placeholder-nohide")) if (formField.tagName == "INPUT" || formField.tagName == "TEXTAREA") formField.dataset.placeholder = formField.placeholder; else formField.dataset.placeholder = formField.innerHTML;
            }));
            ...
            if (forms.length) {
                function changeFormActions(currentInput, dataInInputs, changeInputs) {
                    ...
                    if (currentForm) for (let formIndex = 0; formIndex < forms.length; formIndex++) {
                        ...
                        if (form == currentForm) {
                            ...
                            for (let inputIndex = 0; inputIndex < inputs.length; inputIndex++) {
                                ...
                                if (input == currentInput) {
                                    if (input.value == dataInInputs[formIndex][inputIndex]) changeInputs[inputIndex] = 0; else changeInputs[inputIndex] = 1;
                                    break;
                                }
                            }
                            const buttons = form.querySelectorAll('button[type="submit"]');
                            if (changeInputs.reduce(((acc, number) => acc + number)) > 0) buttons.forEach((button => {
                ...
                document.addEventListener("click", (function(e) {
                    ...
                    if (!currentSelect && e.target.closest(".select")) currentSelect = e.target.closest(".select").querySelector("select");
        ...
        // Валидация форм
            ...
            validateInput(formRequiredItem) {
                ...
                if (formRequiredItem.dataset.required === "email") {
                    ...
                    this.addError(formRequiredItem);
                    error++;
                } else if (formRequiredItem.dataset.required === "tel") if (this.telTest(formRequiredItem)) {
                    this.addError(formRequiredItem);
                    error++;
                } else this.removeError(formRequiredItem); else if (formRequiredItem.dataset.required === "password") if (this.passwordTest(formRequiredItem)) {
                    this.addError(formRequiredItem);
                    error++;
                } else this.removeError(formRequiredItem); else if (formRequiredItem.dataset.required === "code") if (this.codeTest(formRequiredItem)) {
                    this.addError(formRequiredItem);
                    error++;
                } else this.removeError(formRequiredItem); else if (formRequiredItem.dataset.required === "month") if (this.monthTest(formRequiredItem)) {
                    this.addError(formRequiredItem);
                    error++;
                } else this.removeError(formRequiredItem); else if (formRequiredItem.dataset.required === "card-num") if (this.cardNumTest(formRequiredItem)) {
                    this.addError(formRequiredItem);
                    error++;
            ...
            addError(formRequiredItem, errorText = formRequiredItem.dataset.error) {
                ...
                if (formRequiredItem.closest(".form__input")) {
                    ...
                    if (errorText) formRequiredItem.closest(".form__input").insertAdjacentHTML("beforeend", `<div class="form__error">${errorText}</div>`);
                }
            },
        ...
        function formSubmit(options = {
            ...
            // Действия после отправки формы
                ...
                if (form.hasAttribute("data-user-clear")) {
                    const dataUser = JSON.parse(localStorage.getItem("carpet-shop"));
                    const favoriteQuant = dataUser["favorite-quant"] ? +dataUser["favorite-quant"] : 0;
                    const cartQuant = dataUser["cart-quant"] ? +dataUser["cart-quant"] : 0;
                    localStorage.setItem("carpet-shop", JSON.stringify({
                        "favorite-quant": favoriteQuant,
                        "cart-quant": cartQuant,
                        user: {
                            address: {},
                            img: "img/common/not-photo.png",
                            payment: {}
                ...
                inputs.forEach((input => {
                    ...
                    for (const type of inputDataType) {
                        if (!targetObject[type]) targetObject[type] = {};
                ...
                if (form.hasAttribute("data-update-account")) updateAccountInfo();
                if (form.hasAttribute("data-close-form")) closeChangeForm(form);
                ...
                // Переход на другую страницу
                                if (form.hasAttribute("data-href")) window.location.pathname = mainPath + form.getAttribute("data-href");
            }
            function formLogging(message) {
                FLS(`[Формы]: ${message}`);
            }
        }
        /* Модуь формы "колличество" */        function formQuantity() {
            ...
            for (let i = 0; i < quantity.length; i++) {
                ...
                if (value == 1) buttonMinus.classList.add("_disabled"); else buttonMinus.classList.remove("_disabled");
                input.addEventListener("change", (() => {
                    if (input.value < 1) input.value = 1;
                    if (input.value == 1) buttonMinus.classList.add("_disabled"); else buttonMinus.classList.remove("_disabled");
        ...
        /* Модуь звездного рейтинга */        function formRating() {
                ...
                // Возможность указать оценку 
                    ...
                    for (let index = 0; index < ratingItems.length; index++) {
                        ...
                        ratingItem.addEventListener("click", (function(e) {
                            ...
                            // "Отправить" на сервер
                            setRatingValue(ratingItem.value, rating); else {
...
*/
        ...
        class SelectConstructor {
            ...
            selectsActions(e) {
                ...
                if (targetElement.closest(this.getSelectClass(this.selectClasses.classSelect)) || targetElement.closest(this.getSelectClass(this.selectClasses.classSelectTag))) {
                    ...
                    if (targetType === "click") {
                        ...
                        this.selectAction(selectItem); else if (targetElement.closest(this.getSelectClass(this.selectClasses.classSelectOption))) {
            ...
            getSelectedOptionsData(originalSelect, type) {
                ...
                selectedOptions = Array.from(originalSelect.options).filter((option => option.value)).filter((option => option.selected)); else 
        ...
        // Подключение списка активных модулей
        // Подключение модуля
        let inputMasks = document.querySelectorAll("input");
        if (inputMasks.length) modules.inputmask = Inputmask().mask(inputMasks);
 ...
 */
 ...
 // Set attrs
                        for (let i = 0; i < this.length; i += 1) if (arguments.length === 2) 
            // String
            this[i].setAttribute(attrs, value); else 
            // Object
 ...
 // eslint-disable-line
        ...
        shortcut("resize");
        shortcut("scroll");
        /* harmony default export */        // CONCATENATED MODULE: ./node_modules/swiper/shared/dom.js
        const Methods = {
            addClass,
            removeClass,
            hasClass,
...
*/
        ...
        function initSliders() {
			...
			*/
            ...
            if (newProductsSliderBlock) {
                // Указываем скласс нужного слайдера
                // Создаем слайдер
                new core(newProductsSliderBlock, {
                    // Указываем скласс нужного слайдера
                    // Подключаем модули слайдера
                    // для конкретного случая
            ...
            if (discountSliderBlock) {
                // Указываем скласс нужного слайдера
                // Создаем слайдер
                new core(discountSliderBlock, {
                    // Указываем скласс нужного слайдера
                    // Подключаем модули слайдера
                    // для конкретного случая
                    modules: [ Navigation, Pagination, Scrollbar ],
            ...
            if (imageSideSliderBlock) {
                // Указываем скласс нужного слайдера
                // Создаем слайдер
                new core(imageSideSliderBlock, {
                    // Указываем скласс нужного слайдера
                    // Подключаем модули слайдера
                    // для конкретного случая
                    modules: [],
            ...
            if (otherProductSliderBlock) {
                // Указываем скласс нужного слайдера
                // Создаем слайдер
                new core(otherProductSliderBlock, {
                    // Указываем скласс нужного слайдера
                    // Подключаем модули слайдера
                    // для конкретного случая
        ...
        if (sideImages && mainImage) mainImage.src = sideImages[0].src;
        let outPlaceholderInputs = document.querySelectorAll("input[data-out-placeholder]");
        const cityItems = document.querySelectorAll(".city-popup__item");
        document.addEventListener("click", (function(e) {
            outPlaceholderInputs.forEach((outPlaceholderInput => {
                if (e.target.closest(outPlaceholderInput.getAttribute("data-out-placeholder")) || e.target == outPlaceholderInput.nextElementSibling) {
                    if (outPlaceholderInput.value != "") document.querySelector(outPlaceholderInput.getAttribute("data-out-placeholder")).parentElement.classList.add("auto-width"); else document.querySelector(outPlaceholderInput.getAttribute("data-out-placeholder")).parentElement.classList.remove("auto-width");
            ...
            if (sideImages && mainImage) for (const sideImage of sideImages) if (sideImage == e.target) mainImage.src = e.target.src;
            ...
            if (e.target.closest(".item-account__title") && e.target.closest(".item-account__title").closest(".item-account._active")) if (e.target.closest(".item-account__title").classList.contains("form-change-title")) closeChangeForm(e.target.closest(".item-account").querySelector(".item-account__body_change")); else {
                const tabsBlock = e.target.closest("[data-tabs]");
                const tabsNavigation = tabsBlock.querySelector(".tabs-account__navigation");
                if (tabsNavigation) tabsBlock.style.height = tabsNavigation.offsetHeight + "px";
                const tabsItem = tabsBlock.querySelector(".tabs-account__item._active");
                if (tabsItem) tabsItem.classList.remove("_active");
                const account = document.querySelector(".account");
                if (account) account.classList.remove("_mobile-tab-open");
                const tabsTitles = tabsBlock.querySelector("[data-tabs-titles]");
                if (tabsTitles) tabsTitles.classList.remove("_opacity");
            }
            ...
            if (itemType) {
                ...
                if (itemButtonEdit && itemButtonEdit.getAttribute(`data-${itemType}-id`) && itemChangeForm) openChangeForm(itemChangeForm, +itemButtonEdit.getAttribute(`data-${itemType}-id`));
                const itemButtonDelete = e.target.closest(".actions-account__item_delete");
                if (itemButtonDelete && itemButtonDelete.getAttribute(`data-${itemType}-id`)) deleteAccountItem(itemButtonDelete, +itemButtonDelete.getAttribute(`data-${itemType}-id`));
            }
        ...
        paymentItems.forEach((paymentItem => {
            const paymentItemType = paymentItem.getAttribute("data-payment-item-type");
            if (document.querySelector(`.item-title`) && Object.keys(JSON.parse(localStorage.getItem("carpet-shop"))["user"][paymentItemType]).length == 0) document.querySelector(`.item-title`).classList.add("_disabled");
        }));
        function dataFilling(e = false) {
            for (let i = 0; i < Object.keys(JSON.parse(localStorage.getItem("carpet-shop"))).length; i++) {
                ...
                if (!e && elementsForm) elementsForm.forEach((e => {
                    if (!e.hasAttribute("data-dont-remember")) e.value = values[i];
                }));
                let elementsBlock = document.querySelectorAll(`.user-${keys[i]}`);
                if (elementsBlock.length) elementsBlock.forEach((e => {
        ...
        if (products) for (const product of products) {
            ...
            if (productPrices) {
                ...
                if (productPricesActive.length > 2) productPricesActive = productPricesActive.slice(0, 2);
                for (const productPrice of productPricesActive) productPrice.closest(".price-product").classList.add("_active");
        ...
        if (!isMobile.any() && filterAction) filterAction.querySelector(".filter-action__subtitle").innerHTML = document.querySelector(".category__title").innerHTML;
        function fillingFilterAction() {
            const categoryCheckboxes = document.querySelectorAll(".header-filter__checkbox .checkbox__item");
            let checkedTextArr = [];
            if (categoryCheckboxes) for (const categoryCheckbox of categoryCheckboxes) if (categoryCheckbox.querySelector("input").checked) checkedTextArr.push(categoryCheckbox.querySelector(".checkbox__text").innerHTML);
            if (!checkedTextArr.length) checkedTextArr = [ "Не выбрано" ];
            checkedTextArr = checkedTextArr.map(((item, index) => {
                if (index == checkedTextArr.length - 1) return `<span>${item}</span>`; else return `<span>${item},</span>`;
            }));
            let checkedText = checkedTextArr.join("");
            if (filterAction) filterAction.querySelector(".filter-action__subtitle").innerHTML = checkedText;
        }
        fillingFilterAction();
        document.addEventListener("change", (function(e) {
            if (e.target.classList.contains("checkbox__input")) fillingFilterAction();
        }));
        ...
        window.calcPriceTotalProduct = inputQuantity => {
            if (inputQuantity.closest(".cart")) {
                ...
                if (priceTotalProduct && priceProduct) priceTotalProduct.innerHTML = moneyFormat.to(moneyFormat.from(priceProduct.innerHTML) * +inputQuantity.value);
                calcPriceTotal();
            }
        };
        ...
        if (cartBlock) {
            if (+JSON.parse(localStorage.getItem("carpet-shop"))["cart-quant"]) {
                ...
                if (porductItem) for (let index = 1; index < +JSON.parse(localStorage.getItem("carpet-shop"))["cart-quant"]; index++) porductItems.appendChild(porductItem.cloneNode(true));
            } else cartBlock.classList.add("cart_empty");
        ...
        if (favoriteBlock) if (+JSON.parse(localStorage.getItem("carpet-shop"))["favorite-quant"]) {
            ...
            if (porductItem) {
                ...
                for (let index = 1; index < +JSON.parse(localStorage.getItem("carpet-shop"))["favorite-quant"]; index++) porductItems.appendChild(porductItem.cloneNode(true));
            }
        } else favoriteBlock.classList.add("favorite_empty");
        function isLogged() {
            console.log(JSON.parse(localStorage.getItem("carpet-shop"))["user"]["isLogged"]);
            console.log(window.location.pathname);
            if (JSON.parse(localStorage.getItem("carpet-shop"))["user"]["isLogged"] != "true" && window.location.pathname == mainPath + "account.html") window.location.pathname = mainPath + "auth.html";
        }
        isLogged();
            ...
            if (this.document.querySelector(".account")) {
                ...
                tabsBlocks.forEach((tabsBlock => {
                    if (document.documentElement.offsetWidth > 767.98) {
                        ...
                        if (tabsNavigation) if (!tabsActive.length) tabsBlock.style.height = tabsNavigation.scrollHeight + "px"; else tabsNavigation.classList.add("_opacity");
                    }
                }));
            }
        }));
        ...
        DynamicAdapt.prototype.init = function() {
            ...
            // наполнение objects объктами
                ...
                } else object.destination = destination[0];
...
*/
        ...
        /* (i) необходимо для корректного отображения webp из css  */        isWebp();
        /* Добавление класса touch для HTML если браузер мобильный */        addTouchClass();
        /* Добавление  loaded для HTML после полной загрузки страницы */
        // functions.addLoadedClass();
        /* Модуль для работы с меню (Бургер) */        menuInit();
        /* Учет плавающей панели на мобильных устройствах при 100vh */
        // functions.fullVHfix();
        /*
...
*/
        ...
        /* Документация: https://template.fls.guru/template-docs/rabota-s-formami.html */        formFieldsInit({
            viewPass: true
        });
        /* Oтправка формы */
        /* Документация: https://template.fls.guru/template-docs/rabota-s-formami.html */        formSubmit();
        /* Модуль формы "колличество" */        formQuantity();
        /* Модуль звездного рейтинга */        formRating();
